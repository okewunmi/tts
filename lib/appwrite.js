...
export const detectLanguageOfText = async (text) => {
  try {
    if (!text || typeof text !== 'string') {
      throw new Error("Invalid or empty text input");
    }

    const response = await fetch("https://alexneakameni-language-detection.hf.space/run/predict", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Accept": "application/json"
    ...
    if (result && result.data && result.data.length > 0) {
      // Assuming response format is like: [ ["en", score], ["fr", score], ... ]
      return result.data[0][0]; // Return most probable language code
    }
