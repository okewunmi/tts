...
export async function uploadFile(file) {
  ...
  try {
    const uploadedFile = await storage.createFile(
      config.storageId,
      ID.unique(),
      { uri: file.uri, type: file.mimeType } // Pass the file's URI and MIME type
    );
    
    const fileUrl = storage.getFileView(config.storageId, uploadedFile.$id);
    return fileUrl; // Return the URL for use
  } catch (error) {
...
export async function getDocumentById(documentId) {
  try {
    ...
    return document; // Ensure document contains valid keys like `fileUrl`
  } catch (error) {
