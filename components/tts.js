...
// useEffect(() => {
//   // Cleanup oldest audio when queue gets too long
//   if (audioQueue.length > MAX_CONCURRENT_AUDIO) {
//     const toRemove = audioQueue[0];
//     toRemove.unloadAsync().finally(() => {
...
//   // Filter languages for better UX (show matching when typing)
...
//     if (audioQueue.length === 0 || !playing) {
...
//       // Clear previous status handler
//       await nextAudio.setOnPlaybackStatusUpdate(null);

//       // Set new handler
//       await nextAudio.setOnPlaybackStatusUpdate(async (status) => {
...
//       // Unload queued audio
//       const cleanupPromises = audioQueue.map(async (audio) => {
//         try {
...
//   const generateSpeech = async (text, retries = 3) => {
//     for (let attempt = 0; attempt < retries; attempt++) {
//       try {
//         setIsLoading(true);
//         console.log(`Attempt ${attempt + 1}: Sending to Hugging Face TTS API...`);
...
//     if (currentChunk < textChunks.length - 1) {
//       setCurrentChunk(prev => prev + 1);
//       setTimeout(speakCurrentChunk, 300);
//     } else {
//       setPlaying(false);
//       setCurrentChunk(0);
//     }
...
const TTSFunction = ({ text }) => {
  ...
  const generateSpeech = async (text) => {
    try {
      ...
      const response = await fetch("https://dpc-mmstts.hf.space/run/predict", {
        ...
        body: JSON.stringify({ data: [text, selectedLanguage] })
      });
  ...
  const speak = async () => {
    ...
    if (playing) {
      ...
      await audio.setOnPlaybackStatusUpdate(async (status) => {
        if (status.isLoaded) {
  ...
  const restart = async () => {
    if (currentAudio) {
      await currentAudio.setPositionAsync(0);
  ...
  const estimateDuration = (text) => {
    ...
    return ((words / 150) * 60).toFixed(1); // seconds
  };
    ...
    <View style={styles.container}>
      ...
      <Slider
        ...
        onSlidingComplete={async (value) => {
          if (currentAudio) await currentAudio.setPositionAsync(value);
        }}
      />

      <Text style={{ alignSelf: 'center', fontSize: 12, marginBottom: 8 }}>
        Est. Duration: {estimateDuration(text)}s
      </Text>
...
const styles = StyleSheet.create({
  container: { backgroundColor: "#fff", width: "100%", paddingVertical: 10 },
  languageContainer: {
    flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, marginBottom: 10,
  },
  label: { fontWeight: '700', marginRight: 10, fontSize: 13 },
  pickerContainer: {
    flex: 1, borderWidth: 1, borderColor: '#ddde', borderRadius: 8, overflow: 'hidden',
  },
  picker: { height: 50, width: '100%' },
  controlsRow: {
    flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center', paddingHorizontal: 10,
  },
  controlButton: { alignItems: 'center', justifyContent: 'center', width: 50 },
  controlText: {
    fontWeight: "800", fontSize: 10, marginTop: 4, color: '#9E9898',
  },
  playButton: {
    justifyContent: "center", alignItems: "center", borderRadius: 30, height: 55, width: 55, backgroundColor: "#3273F6",
  },
  speedContainer: {
    height: 40, width: 40, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#9E9898', borderRadius: 20,
  },
  speedText: { fontWeight: '600', fontSize: 12 },
});
