...
// const AUDIO_CACHE_DIR = FileSystem.cacheDirectory + 'tts_audio/';
...
// const getCachedAudioPath = async (text) => {
//   const hash = getHash(text);
//   const filePath = AUDIO_CACHE_DIR + hash + '.mp3';
//   const exists = await FileSystem.getInfoAsync(filePath);
//   return exists.exists ? filePath : null;
// };

// const cacheAudio = async (text, base64Data) => {
//   const hash = getHash(text);
//   const filePath = AUDIO_CACHE_DIR + hash + '.mp3';
...
//       const audioUrl = `https://dpc-mmstts.hf.space/file=${result.data[0].name}`;
//       const download = FileSystem.createDownloadResumable(audioUrl, AUDIO_CACHE_DIR + 'temp.mp3');
...
//       const cached = await getCachedAudioPath(nextText);

//       if (cached) {
//         chunkAudios.current[nextIndex] = cached;
//       } else {
//         try {
//           const base64 = await generateAudioFromText(nextText);
//           if (base64) {
//             const filePath = await cacheAudio(nextText, base64);
//             chunkAudios.current[nextIndex] = filePath;
//           }
//         } catch (error) {
...
//       // Load first few chunks in parallel
//       const initialLoads = [];
...
//           const cached = await getCachedAudioPath(chunkText);
//           if (cached) {
//             chunkAudios.current[i] = cached;
//           } else {
//             const base64 = await generateAudioFromText(chunkText);
//             if (base64) {
//               const filePath = await cacheAudio(chunkText, base64);
//               chunkAudios.current[i] = filePath;
//             }
//           }
//         })());
//       }
...
//           <Text style={styles.controlText}>Speed</Text>
//         </TouchableOpacity>
//         <TouchableOpacity style={styles.controlButton} onPress={downloadAudio}>
//           <FontAwesome6 name="download" size={22} color="#9E9898" />
